...
mu       = 3;                            % Coeficiente de aversão ao risco;
beta     = 0.96;                         % Fator de desconto;
phi      = 1;                            % Limite de endividamento; 
rho      = 0.30;                         % Parâmetro de persistência dos choques de renda;
sigma_e  = sqrt(0.01*(1-rho^2));         % Variância do erro do processo de renda.
...
% FUNCTION: [s, Pi] = mytauchen(const,m,rho,sigma_e,N)
%
% INPUTS:
%   const:  scalar, intercept term of the AR(1) process
%   rho:    scalar, AR-coefficient
%   sigma_e:    scalar, standard deviation of innovations
...
[S, P] = mytauchen(const, m, rho, sigma_e, ny);
...
lambda_wide = reshape(lambda, na, ny);
psi = sum(lambda_wide,2);
